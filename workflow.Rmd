...
extractPeaks <- function(flow, mintimeDiff = 7) {
  peaks <- c()
  last_peak <- -Inf
  for (i in 2:(length(flow) - 1)) {
    if (flow[i] > flow[i - 1] && flow[i] > flow[i + 1]) {
      if (i - last_peak >= mintimeDiff) {
        peaks <- c(peaks, i)
        last_peak <- i
      }
    }
...
peaks <- extractPeaks(lf_flowPOT$flow, mintimeDiff = 7)
# Filter rows where flow > threshold and index is in peaks
pot_raw <- lf_flow[(lf_flow$flow > lf_flow$threshold) & (seq_along(lf_flow$flow) %in% peaks), ]
# Save to CSV
