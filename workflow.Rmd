...
# Summarise thresholds by month
vmt_summary <- lf_flow %>% 
  dplyr::group_by(month) %>% 
  dplyr::summarise(threshold=mean(threshold))

# Prepare flow data for peak extraction
lf_flowPOT <- lf_flow
lf_flowPOT$flow[is.na(lf_flowPOT$flow)] <- -9999

# Define time and observation vectors
vecTime <- as.numeric(lf_flowPOT$date)  # or use seq_along if date is not numeric
vecObs <- lf_flowPOT$flow

# Extract peaks using custom function
peaks <- extractPeaksX(vecTime = vecTime, vecObs = vecObs, mintimeDiff = 73, thrConst = 2/3, threshold = 0)

# Filter POT events
pot_raw <- lf_flow[(lf_flow$flow > lf_flow$threshold) & (seq_along(lf_flow$flow) %in% which(peaks == 1)), ]
